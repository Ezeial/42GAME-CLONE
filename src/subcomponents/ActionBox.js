...
const AskBox = ({ reset, functionNumber, i }) => {
  ...
  useEffect(() => {
    if (
      toggleL.toggled.find((bool) => bool === true) &&
      toggleR.toggled.find((bool) => bool === true)
    )
      reset();
...
const SelectButton = ({
  ...
  return (
    <SelectButtonContainer
      onClick={() => toggle(i)}
      toggled={toggled[i]}
      bg={instruction.color}
    >
      {i === toggled.indexOf(true) && (
        <AskBox i={i} functionNumber={functionNumber} reset={reset} />
      )}
      {instruction.move && !instruction.move.startsWith("F0") && (
        <Svg src={Move} rotate={moveEnum[instruction.move]} />
      )}
...
const FunctionBox = ({ func, functionNumber }) => {
  ...
  return (
    <FunctionBoxContainer>
      <SelectButtonContainer color="#EEE" bg="#222">
        F{functionNumber}
      </SelectButtonContainer>
      {func.map((instruction, i) => (
        <SelectButton
          key={i}
...
function ActionBox() {
  ...
  return (
    <Container>
      {actions.map((func, i) => (
        <FunctionBox key={i} functionNumber={i} func={func} />
      ))}
    </Container>
  );
}
